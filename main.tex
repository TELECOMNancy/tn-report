...
\def\reportAuthorFirstName{Jon}
\def\reportAuthorLastName{Snow}
\def\reportStudentId{20140410}


\def\reportTitle{Winter is Coming}
\def\reportLongTitle{Winter is Coming -- You know nothing Jon Snow}
\def\reportYear{2013--2014}

\def\reportAuthorAddress{The Wall, Caste Black}
\def\reportAuthorPhone{numéro rue, VILLE, code postal}
\def\reportAuthorEmail{jon@castleblack.com}

\def\reportCompany{Home Box Office}
\def\reportCompanyAddress{numéro rue, VILLE, code postal}
\def\reportCompanyPhone{téléphone}

\def\reportReponsible{Eddard Stark}
\def\reportIndustrialSupervisor{Eddard Stark}
\def\reportAcademicSupervisor{Tyrion Lannister}

\def\place{Winterfell}
\def\date{\today}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=10cm]{figures/telecom_nancy-logo}
  \caption{Logo de TELECOM Nancy}
  \label{fig:logo-tn}
\end{figure}

La Figure~\ref{fig:logo-tn} représente le logo de TELECOM Nancy.
...
\bibliography{references} % See references.bib 
